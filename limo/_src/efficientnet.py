...
class SqueezeExcite(linen.Module):
    ...
    def __call__(self, x: chex.Array) -> chex.Array:
        ...
        x_se = self.conv_layer(rd_features, (1, 1), bias=True, name="conv_reduce")(x_se)
...
class DepthwiseSeparableConv(linen.Module):
    ...
    def __call__(self, x: chex.Array) -> chex.Array:
        ...
        x = self.conv_layer(self.features, (self.pw_kernel_size,) * 2, name="conv_pw")(x)
...
class InvertedResidual(linen.Module):
    ...
    def __call__(self, x: chex.Array) -> chex.Array:
        ...
        x = self.conv_layer(mid_features, (self.exp_kernel_size,) * 2, name="conv_pw")(x)
        x = self.norm_layer(name="bn1")(x)
        x = self.act_layer(name="bn1.act")(x)
        ...
        x = self.conv_layer(self.features, (self.pw_kernel_size,) * 2, name="conv_pwl")(x)
...
class EfficientNet(linen.Module):
    ...
    @linen.compact
    def __call__(self, x: chex.Array) -> chex.Array:
        x = self.conv_layer(self.stem_size, (3, 3), 2, name="conv_stem")(x)
        x = self.norm_layer(name="bn1")(x)
        x = self.act_layer(name="bn1.act")(x)
        ...
        x = self.conv_layer(self.features, (1, 1), name="conv_head")(x)
        x = self.norm_layer(name="bn2")(x)
        x = self.act_layer(name="bn2.act")(x)
        ...
        if self.num_classes > 0:
            x = layers.Dense(self.num_classes, name="classifier")
        return x
...
efficientnet_b0 = partial(_efficientnet, feature_multiplier=1.0, depth_multiplier=1.0, dropout=0.2)
